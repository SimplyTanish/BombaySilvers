...
import { Check, FileText, Upload, ShieldCheck } from "lucide-react";
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="glass rounded-3xl p-6 sm:p-8">
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <p className="mt-1 max-w-lg text-sm text-muted-foreground">
                Upload statutory documents. Each file is encrypted at rest and only accessible by compliance officers.
              </p>
            </div>
            <div className="hidden text-right sm:block">
              <div className="metallic-text font-mono text-2xl font-semibold">4 / 6</div>
              <div className="text-[11px] uppercase tracking-widest text-muted-foreground">verified</div>
            </div>
          </div>
            ...
            {docs.map((d) => (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <div className="truncate font-mono text-xs text-muted-foreground">{d.meta}</div>
                </div>
          ...
          <div className="mt-6 rounded-2xl border border-dashed border-border/70 bg-[var(--surface-2)]/40 p-6 text-center">
            ...
            <div className="mt-1 text-xs text-muted-foreground">PDF, JPG, PNG · up to 10 MB each</div>
          </div>

          <div className="mt-8 flex items-center justify-between border-t border-border/60 pt-6">
            <Link to="/onboarding" className="text-sm text-muted-foreground hover:text-foreground">← Back</Link>
            <Link
              to="/dashboard"
...
function StatusPill({ status }: { status: string }) {
  const map: Record<string, { c: string; label: string }> = {
    verified: { c: "text-[var(--gain)] border-[var(--gain)]/30 bg-[var(--gain)]/10", label: "Verified" },
    processing: { c: "text-[var(--warn)] border-[var(--warn)]/30 bg-[var(--warn)]/10", label: "Processing" },
    pending: { c: "text-muted-foreground border-border/60 bg-[var(--surface-3)]", label: "Pending" },
  };
  const s = map[status];
  return (
    <span className={"inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] " + s.c}>
