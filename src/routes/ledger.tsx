      ...
      <div className="grid gap-4 lg:grid-cols-4">
        ...
        <GlassCard className="p-5 lg:col-span-3">
          <div className="flex items-center justify-between">
            <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Credit vs debit · last 6 months</div>
            ...
            {byMonth.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={byMonth} barCategoryGap={16}>
                  ...
                  <YAxis tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(v: number) => `${Math.round(v / 100000)}L`} />
                  <Tooltip
      ...
      <GlassCard className="mt-6 overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-border/40">
            {entries!.map((t) => (
              <div key={t.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 p-4 hover:bg-[var(--surface-2)]/60">
                ...
                <div className="flex items-center gap-3">
                  <span className={"font-mono text-sm " + (t.entry_type === "credit" ? "text-[var(--gain)]" : "text-[var(--loss)]")}>
...
function useMemoMonthlyBreakdown(entries: { entry_type: string; amount: number; created_at: string }[]) {
  return useMemo(() => {
