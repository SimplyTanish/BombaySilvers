      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <GlassCard className="relative overflow-hidden p-6 lg:col-span-2">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-2">
            ...
            <button onClick={() => void copy()} className="inline-flex items-center gap-1.5 rounded-xl border border-border/70 bg-[var(--surface-2)] px-3 py-2 text-xs hover:bg-[var(--surface-3)]">
              {copied ? <Check className="h-3.5 w-3.5 text-[var(--gain)]" /> : <Copy className="h-3.5 w-3.5" />}
              Copy
            </button>
            <button onClick={() => void share()} className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-b from-[#f1f1f4] to-[#b6b7bb] px-3 py-2 text-xs font-medium text-black">
        ...
        <div className="grid gap-4">
          <Stat i={Users2} l="Total referrals" v={stats ? String(stats.total) : "—"} sub={stats ? `${stats.active} active this quarter` : "Loading"} />
          <Stat i={Gift} l="Total earned" v={stats ? `₹${fmtINR(stats.earned)}` : "—"} sub="Credited to ledger" tone="gain" />
          <Stat i={TrendingUp} l="Pending invites" v={stats ? String(stats.pending) : "—"} sub="Awaiting KYC completion" />
        </div>
      </div>

      <GlassCard className="mt-6 overflow-hidden">
        <div className="border-b border-border/60 p-4 text-sm font-medium">Referred dealers</div>
        {isLoading && <div className="p-8 text-center text-sm text-muted-foreground">Loading referrals…</div>}
        {isError && <div className="p-8 text-center text-sm text-[var(--loss)]">Could not load referrals.</div>}
        ...
        <div className="divide-y divide-border/40">
          {(data?.rows ?? []).map((r) => (
            <div key={r.id} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-4 p-4">
              <div className="grid h-10 w-10 place-items-center rounded-full bg-gradient-to-br from-[#d9d9dd] to-[#7a7b7f] font-mono text-xs font-bold text-black">
                {r.referred_name.split(" ").map((w) => w[0]).slice(0, 2).join("").toUpperCase()}
              </div>
              <div className="min-w-0">
                <div className="truncate text-sm font-medium">{r.referred_name}</div>
                <div className="text-xs text-muted-foreground">
                  {r.referred_phone} · {r.referred_dealer?.dealer_code ?? "not registered yet"} · joined{" "}
                  {new Intl.DateTimeFormat("en-IN", { month: "short", year: "numeric" }).format(new Date(r.created_at))}
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className={"rounded-full px-2.5 py-1 text-[11px] " + (statusTone[r.status] ?? statusTone.pending)}>
